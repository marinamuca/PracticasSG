...
class reloj extends THREE.Object3D {
  constructor(gui,titleGui) {
    ...
    var sphereMat = new THREE.MeshPhongMaterial({color : 0x0000cc})
    this.blueSphere = new THREE.Mesh(sphereGeom, sphereMat)
  ...
  createGUI (gui,titleGui) {
    ...
    folder.add (this.guiControls, 'parar', true, false).name("Parar Reloj:");
  ...
  update () {
    ...
    if(this.guiControls.parar)
      var velocidad = 0;
    else
      var velocidad = 1
    this.redSphere.rotation.y -= velocidad* this.segundos * ((6 * Math.PI) / 180)
    this.blueSphere.rotation.y -= velocidad* this.segundos * ((0.1 * Math.PI) / 180)
